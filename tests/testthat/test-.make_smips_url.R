test_that("A valid url part is created", {
  ...
  expect_identical(.make_smips_url("totalbucket", date_),
                   "smips_totalbucket_mm_20200101.tif")
  expect_identical(.make_smips_url("SMindex", date_),
                   "smips_smi_perc_20200101.tif")
  expect_identical(.make_smips_url("bucket1", date_),
                   "smips_bucket1_mm_20200101.tif")
  expect_identical(.make_smips_url("bucket2", date_),
                   "smips_bucket2_mm_20200101.tif")
  expect_identical(.make_smips_url("deepD", date_),
                   "smips_deepD_mm_20200101.tif")

  # different date
  expect_identical(.make_smips_url("runoff", as.Date("2020-01-02")),
                   "smips_runoff_mm_20200102.tif")
})

test_that("An invalid collection throws an error", {
  valid_collections <- c("totalbucket",
                         "SMindex",
                         "bucket1",
                         "bucket2",
                         "deepD",
  ...
  error_msg <- paste0("`.collection` must be one of ",
                      valid_collections_str,
                      ', not "asdf".')
