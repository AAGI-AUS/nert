...
test_that(".normalise_smips_collection returns the full SMIPS set for NULL/'all'", {
  ...
  expect_identical(length(.normalise_smips_collection(NULL)), length(valid_smips))
  expect_identical(sort(.normalise_smips_collection(NULL)), sort(valid_smips))
  expect_identical(.normalise_smips_collection("all"), .normalise_smips_collection(NULL))
})
...
test_that(".normalise_asc_collection returns the full ASC set for NULL/'all'", {
  ...
  expect_identical(.normalise_asc_collection("all"), .normalise_asc_collection(NULL))
})
...
test_that(".normalise_aet_collection returns the full AET set for NULL/'all'", {
  ...
  expect_identical(.normalise_aet_collection("all"), .normalise_aet_collection(NULL))
})
...
test_that(".normalise_soildiv_collection returns the full SOILDIV set for NULL/'all'", {
  ...
  expect_identical(length(.normalise_soildiv_collection(NULL)), length(valid_soildiv))
  expect_identical(sort(.normalise_soildiv_collection(NULL)), sort(valid_soildiv))
  expect_identical(.normalise_soildiv_collection("all"), .normalise_soildiv_collection(NULL))
})

test_that(".normalise_soildiv_collection deduplicates", {
  expect_identical(
    .normalise_soildiv_collection(c("Bacteria_NMDS1", "Bacteria_NMDS2", "Bacteria_NMDS1")),
...
test_that(".normalise_soildiv_collection preserves order", {
  expect_identical(
    .normalise_soildiv_collection(c("Bacteria_NMDS2", "Fungi_NMDS1", "Bacteria_NMDS1")),
...
test_that(".normalise_soildiv_collection warns and filters invalid variants", {
  expect_warning(
    out <- .normalise_soildiv_collection(c("bacteria_nmds1", "bac_nmds2", "Fungi_NMDS3")),
    "Invalid"
  )
...
test_that(".normalise_soildiv_collection errors when no valid variants remain", {
  suppressWarnings(
    expect_error(.normalise_soildiv_collection(c("bac_NMDS1", "funNMDS2")), "No valid")
  )
})
...
test_that("work-item planner emits one item per SMIPS (date, variant)", {
  ...
  items <- .build_work_items("SMIPS", dates, smips_collection = .normalise_smips_collection(NULL))
...
test_that("work-item planner emits one item per AET (date, variant)", {
  ...
  items <- .build_work_items("AET", dates, aet_collection = .normalise_aet_collection(NULL))
...
test_that("work-item planner emits 14 items for all SLGA datasets at one depth, one stat", {
  ...
  expect_true(all(grepl("_EV_000_005", vapply(items, function(x) x$cols[1L],
                                        character(1L)))))
})
...
test_that("work-item planner emits one item per ASC variant", {
  items <- .build_work_items("ASC", as.Date("2022-02-03"), asc_collection = .normalise_asc_collection(NULL))
  # 2 variants = 2 items
  expect_identical(length(items), 2L)
  expect_true(is.na(items[[1]]$date_idx))
  expect_true(is.na(items[[2]]$date_idx))
  expect_true(all(grepl("^ASC_", vapply(items, function(x) x$cols[1L],
                                        character(1L)))))
})
...
test_that("work-item planner emits one item per SOILDIV variant", {
  items <- .build_work_items("SOILDIV", as.Date("2021-12-21"),
  ...
  expect_true(all(grepl("^SOILDIV_", vapply(items, function(x) x$cols[1L],
                                        character(1L)))))
})
...
test_that("work-item planner emits one item per PHENOLOGY (year,variant)", {
  ...
  expect_true(all(grepl("^PHENOLOGY_", vapply(items, function(x) x$cols[1L],
                                        character(1L)))))
})
...
test_that("AET work items have ETa numeric-typed, pixel_qa numeric-typed", {
  items <- .build_work_items("AET", as.Date("2024-01-01"),
...
test_that("collect_tern_data plans schema independently of fetch success", {
  ...
  expect_setequal(cols, c(
    "SMIPS_bucket1", "SMIPS_bucket2",
    "AWC_EV_000_005", "AWC_EV_060_100",
    "CANOPY",
    "ASC_EV", "ASC_CI"
  ))
})
