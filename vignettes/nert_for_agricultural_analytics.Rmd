...
First, we model the grain yield simply, without any reference to the SMIPS 
data that we have downloaded. In this case we will use the **mgcv** package 
to model the grain yield with a GAM--taking the Variety, Nitrogen rate and
seeding rate as fixed terms, and incorporating the Site and replicate
structure of the experiment in a smooth random effect term.

```{r simplemodel}
library(mgcv)
simple_model <- bam(
  Yield_Tha ~ Variety * Nitrogen_kgNha * SeedRate_plantsm2
    + s(Site, Rep, bs = "re"),
  data = grain
)

summary(simple_model)$r.sq
plot(simple_model, all.terms = TRUE)
```

TODO: Final paragraph here re: the simple model.
...
We can then add this average soil moisture as a fixed effect to the GAM model:

```{r augmentedmodel}
augmented_model <- bam(
  Yield_Tha ~ Variety * Nitrogen_kgNha * SeedRate_plantsm2 + SoilMoisture_avg
    + s(Site, Rep, bs = "re"),
  data = grain
)

summary(augmented_model)$r.sq
plot(augmented_model, all.terms = TRUE)
```

TODO: Final paragraph here re: the augmented model. Ideally we expect that
it makes the effect of Nitrogen easier to isolate.
